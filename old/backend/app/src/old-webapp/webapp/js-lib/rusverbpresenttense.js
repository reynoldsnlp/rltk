	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewhit').addClass('colorizeStyleRusVerbPresentTense');
	},
	
	colorizeSpan: function(span, topic, index) {
		span.find('span.wertiviewhit').addClass('colorizeStyleRusVerbPresentTense');
	},
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.rusverbpresenttense.clickHandler)); 
	},
	...
	mc: function(contextDoc) {
		...
		var hitList = [];
		var tokens = [];
		wertiview.rusverbpresenttense.types = [];
		...
		wertiview.rusverbpresenttense.maxLength = wertiview.rusverbpresenttense.MAX_MC;
		wertiview.activity.mc(contextDoc, hitList, 
		wertiview.rusverbpresenttense.clozeInputHandler, 
		wertiview.rusverbpresenttense.clozeHintHandler, 
		wertiview.rusverbpresenttense.mcGetOptions, 
		wertiview.rusverbpresenttense.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.rusverbpresenttense.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.rusverbpresenttense.types);
		        
		        // Add the distractor forms to the options list:
		        while (j < wertiview.rusverbpresenttense.types.length && options.length < wertiview.rusverbpresenttense.MAX_MC - 1) {
		            // The forms that are homonymous to the correct form are excluded from the list of options:
		            if (wertiview.rusverbpresenttense.types[j].toLowerCase() != $hit.attr('correctForm').toLowerCase() && wertiview.rusverbpresenttense.types[j] != "") 
		            {
		               options.push(wertiview.lib.matchCapitalization(wertiview.rusverbpresenttense.types[j], capType)); 
		            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.rusverbpresenttense.clozeHintHandler, 
		wertiview.rusverbpresenttense.mcGetCorrectAnswer,
		wertiview.rusverbpresenttense.clozeAddBaseform);
	},
