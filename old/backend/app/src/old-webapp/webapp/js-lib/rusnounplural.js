	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewhit').addClass('colorizeStyleRusNounPlural');
	},
	
	colorizeSpan: function(span, topic, index) {
		span.find('span.wertiviewhit').addClass('colorizeStyleRusNounPlural');
	},
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.rusnounplural.clickHandler)); 
	},
	...
	mc: function(contextDoc) {
		...
		wertiview.rusnounplural.maxLength = wertiview.rusnounplural.MAX_MC;
		wertiview.activity.mc(contextDoc, hitList, 
				wertiview.rusnounplural.clozeInputHandler, 
				wertiview.rusnounplural.clozeHintHandler, 
				wertiview.rusnounplural.mcGetOptions, 
				wertiview.rusnounplural.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.rusnounplural.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.rusnounplural.types);
        
        // Add the distractor forms to the options list:
        while (j < wertiview.rusnounplural.types.length && options.length < wertiview.rusnounplural.MAX_MC - 1) {
            // The forms that are homonymous to the correct form are excluded from the list of options:
            if (wertiview.rusnounplural.types[j].toLowerCase() != $hit.attr('correctForm').toLowerCase() && wertiview.rusnounplural.types[j] != "") 
            {
               options.push(wertiview.lib.matchCapitalization(wertiview.rusnounplural.types[j], capType)); 
            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.activity.cloze(contextDoc, hitList, 
				...
				wertiview.rusnounplural.clozeHintHandler, 
				wertiview.rusnounplural.mcGetCorrectAnswer,
				wertiview.rusnounplural.clozeAddBaseform);
	},
