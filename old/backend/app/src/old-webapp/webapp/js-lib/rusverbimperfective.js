	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.rusverbimperfective.clickHandler)); 
	},
	...
	mcGetOptions: function($hit, capType){
	    ...
	    wertiview.lib.shuffleList(wertiview.rusverbimperfective.types);
		        ...
		        while (j < wertiview.rusverbimperfective.types.length && options.length < wertiview.rusverbimperfective.MAX_MC - 1) {
		            ...
		            if (wertiview.rusverbimperfective.types[j].toLowerCase() != $hit.attr('correctForm').toLowerCase() && wertiview.rusverbimperfective.types[j] != "") 
		            {
