	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewhit').addClass('colorizeStyleRusAdjectiveNeutral');
	},
	
	colorizeSpan: function(span, topic, index) {
		span.find('span.wertiviewhit').addClass('colorizeStyleRusAdjectiveNeutral');
	},
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.rusadjectiveneutral.clickHandler)); 
	},
	...
	mc: function(contextDoc) {
		...
		var hitList = [];
		var tokens = [];
		wertiview.rusadjectiveneutral.types = [];
		...
		wertiview.rusadjectiveneutral.maxLength = wertiview.rusadjectiveneutral.MAX_MC;
		wertiview.activity.mc(contextDoc, hitList, 
				wertiview.rusadjectiveneutral.clozeInputHandler, 
				wertiview.rusadjectiveneutral.clozeHintHandler, 
				wertiview.rusadjectiveneutral.mcGetOptions, 
				wertiview.rusadjectiveneutral.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.rusadjectiveneutral.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.rusadjectiveneutral.types);
        
        // Add the distractor forms to the options list:
        while (j < wertiview.rusadjectiveneutral.types.length && options.length < wertiview.rusadjectiveneutral.MAX_MC - 1) {
            // The forms that are homonymous to the correct form are excluded from the list of options:
            if (wertiview.rusadjectiveneutral.types[j].toLowerCase() != $hit.attr('correctForm').toLowerCase() && wertiview.rusadjectiveneutral.types[j] != "") 
            {
               options.push(wertiview.lib.matchCapitalization(wertiview.rusadjectiveneutral.types[j], capType)); 
            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.activity.cloze(contextDoc, hitList, 
				...
				wertiview.rusadjectiveneutral.clozeHintHandler, 
				wertiview.rusadjectiveneutral.mcGetCorrectAnswer,
				wertiview.rusadjectiveneutral.clozeAddBaseform);
	},
