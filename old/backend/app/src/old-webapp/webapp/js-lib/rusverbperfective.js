	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewhit').addClass('colorizeStyleRusVerbPerfective');
	},
	
	colorizeSpan: function(span, topic, index) {
		span.find('span.wertiviewhit').addClass('colorizeStyleRusVerbPerfective');
	},
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.rusverbperfective.clickHandler)); 
	},
	...
	mc: function(contextDoc) {
		...
		var hitList = [];
		var tokens = [];
		wertiview.rusverbperfective.types = [];
		...
		wertiview.rusverbperfective.maxLength = wertiview.rusverbperfective.MAX_MC;
		wertiview.activity.mc(contextDoc, hitList, 
		wertiview.rusverbperfective.clozeInputHandler, 
		wertiview.rusverbperfective.clozeHintHandler, 
		wertiview.rusverbperfective.mcGetOptions, 
		wertiview.rusverbperfective.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.rusverbperfective.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.rusverbperfective.types);
		        
		        // Add the distractor forms to the options list:
		        while (j < wertiview.rusverbperfective.types.length && options.length < wertiview.rusverbperfective.MAX_MC - 1) {
		            // The forms that are homonymous to the correct form are excluded from the list of options:
		            if (wertiview.rusverbperfective.types[j].toLowerCase() != $hit.attr('correctForm').toLowerCase() && wertiview.rusverbperfective.types[j] != "") 
		            {
		               options.push(wertiview.lib.matchCapitalization(wertiview.rusverbperfective.types[j], capType)); 
		            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.rusverbperfective.clozeHintHandler, 
		wertiview.rusverbperfective.mcGetCorrectAnswer,
		wertiview.rusverbperfective.clozeAddBaseform);
	},
