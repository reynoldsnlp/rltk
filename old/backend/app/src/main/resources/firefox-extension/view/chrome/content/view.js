...
(function() {
	...
	this.shutdown = function() {
		...
		// reset the 'always on' option to 'off' when the browser is closed
...
wertiview.ns(function() {
	...
	load: function(context){
		...
		loader.loadSubScript("chrome://view/content/jquery-1.8.3.min.js", context);
		...
		var uriVIEW = ios.newURI("chrome://view/content/view.css", null, null);
		if(!sss.sheetRegistered(uriVIEW, sss.USER_SHEET)) {
			sss.loadAndRegisterSheet(uriVIEW, sss.USER_SHEET);
		}
	...
	// return the preference 'ajaxTimeout'
	getAjaxTimeout: function() {
		var prefservice = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService);
		var prefs = prefservice.getBranch("extensions.wertiview.");
		
		var ajaxTimeout = prefs.getIntPref("ajaxTimeout");
		return ajaxTimeout;
	},
	
	// return the preference 'showInst'
	getShowInst: function() {
		var prefservice = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService);
		var prefs = prefservice.getBranch("extensions.wertiview.");
		
		var showInst = prefs.getBoolPref("showInst");
		return showInst;
	},
	...
	setUserid: function(userid) {
		...
		prefs.setCharPref("userid", userid);
	},	
	
	disableInst: function() {
		var prefservice = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService);
		var prefs = prefservice.getBranch("extensions.wertiview.");

		prefs.setBoolPref("showInst", false);
	},
