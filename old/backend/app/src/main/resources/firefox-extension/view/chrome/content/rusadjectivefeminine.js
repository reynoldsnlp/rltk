wertiview.ns(function() {
	wertiview.rusadjectivefeminine = {
	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewhit').addClass('colorizeStyleRusAdjectiveFeminine');
	},
	
	colorizeSpan: function(span, topic, index) {
		span.find('span.wertiviewhit').addClass('colorizeStyleRusAdjectiveFeminine');
	},
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.rusadjectivefeminine.clickHandler)); 
	},
	...
	mc: function(contextDoc) {
		...
		var hitList = [];
		var tokens = [];
		wertiview.rusadjectivefeminine.types = [];
		...
		wertiview.rusadjectivefeminine.maxLength = wertiview.rusadjectivefeminine.MAX_MC;
		wertiview.activity.mc(contextDoc, hitList, 
				wertiview.rusadjectivefeminine.clozeInputHandler, 
				wertiview.rusadjectivefeminine.clozeHintHandler, 
				wertiview.rusadjectivefeminine.mcGetOptions, 
				wertiview.rusadjectivefeminine.mcGetCorrectAnswer);
	},
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.rusadjectivefeminine.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.rusadjectivefeminine.types);
        
        // Add the distractor forms to the options list:
        while (j < wertiview.rusadjectivefeminine.types.length && options.length < wertiview.rusadjectivefeminine.MAX_MC - 1) {
            // The forms that are homonymous to the correct form are excluded from the list of options:
            if (wertiview.rusadjectivefeminine.types[j].toLowerCase() != $hit.attr('correctForm').toLowerCase() && wertiview.rusadjectivefeminine.types[j] != "") 
            {
               options.push(wertiview.lib.matchCapitalization(wertiview.rusadjectivefeminine.types[j], capType)); 
            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.activity.cloze(contextDoc, hitList, 
				...
				wertiview.rusadjectivefeminine.clozeHintHandler, 
				wertiview.rusadjectivefeminine.mcGetCorrectAnswer,
				wertiview.rusadjectivefeminine.clozeAddBaseform);
	},
