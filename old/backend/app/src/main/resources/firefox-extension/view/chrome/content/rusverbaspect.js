wertiview.ns(function() {
	wertiview.rusverbaspect = {
		...
		colorize: function(contextDoc) {
			...
			$('span.wertiviewhit[aspect="Perf"]').addClass('colorizeStyleRusVerbPerfectiveAspect');
			$('span.wertiviewhit[aspect="Impf"]').addClass('colorizeStyleRusVerbImperfectiveAspect');
		},
		
		colorizeSpan: function(span, topic, index) {
			span.find('span.wertiviewhit[aspect="Perf"]').addClass('colorizeStyleRusVerbPerfectiveAspect');
			span.find('span.wertiviewhit[aspect="Impf"]').addClass('colorizeStyleRusVerbImperfectiveAspect');
		},
		...
		click: function(contextDoc) {
			...
			// handle click
			$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.rusverbaspect.clickHandler)); 
		},
		...
		mc: function(contextDoc) {
			...
			wertiview.rusverbaspect.maxLength = wertiview.rusverbaspect.MAX_MC;
			wertiview.activity.mc(contextDoc, hitList, 
			wertiview.rusverbaspect.clozeInputHandler, 
			wertiview.rusverbaspect.clozeHintHandler, 
			wertiview.rusverbaspect.mcGetOptions, 
			wertiview.rusverbaspect.mcGetCorrectAnswer);
		...
		mcGetOptions: function($hit, capType){
			...
			wertiview.rusverbaspect.types = $hit.attr('distractors').split(" ");
		    wertiview.lib.shuffleList(wertiview.rusverbaspect.types);
			        
			        // Add the distractor forms to the options list:
			        while (j < wertiview.rusverbaspect.types.length && options.length < wertiview.rusverbaspect.MAX_MC - 1) {
			            // The forms that are homonymous to the correct form are excluded from the list of options:
			            if (wertiview.rusverbaspect.types[j].toLowerCase() != $hit.attr('correctForm').toLowerCase() && wertiview.rusverbaspect.types[j] != "") 
			            {
			               options.push(wertiview.lib.matchCapitalization(wertiview.rusverbaspect.types[j], capType)); 
			            }
				j++;
			}
		...
		cloze: function(contextDoc) {
			...
			wertiview.rusverbaspect.clozeHintHandler, 
			wertiview.rusverbaspect.mcGetCorrectAnswer,
			wertiview.rusverbaspect.clozeAddBaseform);
		},
