wertiview.ns(function() {
	wertiview.rusparticiples = {
		...
		colorize: function(contextDoc) {
			...
			$('span.wertiviewhit').addClass('colorizeStyleRusParticiples');
		},
		
		colorizeSpan: function(span, topic, index) {
			span.find('span.wertiviewhit').addClass('colorizeStyleRusParticiples');
		},
		...
		click: function(contextDoc) {
			...
			// handle click
			$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.rusparticiples.clickHandler)); 
		},
		...
		mc: function(contextDoc) {
			...
			var hitList = [];
			var tokens = [];
			wertiview.rusparticiples.types = [];
			...
			wertiview.rusparticiples.maxLength = wertiview.rusparticiples.MAX_MC;
			wertiview.activity.mc(contextDoc, hitList, 
					wertiview.rusparticiples.clozeInputHandler, 
					wertiview.rusparticiples.clozeHintHandler, 
					wertiview.rusparticiples.mcGetOptions, 
					wertiview.rusparticiples.mcGetCorrectAnswer,
					wertiview.rusparticiples.clozeAddBaseform);
		...
		mcGetOptions: function($hit, capType){
			...
			wertiview.rusparticiples.types = $hit.attr('distractors').split(" ");
		    wertiview.lib.shuffleList(wertiview.rusparticiples.types);
	        
	        // Add the distractor forms to the options list:
	        while (j < wertiview.rusparticiples.types.length && options.length < wertiview.rusparticiples.MAX_MC - 1) {
	            // The forms that are homonymous to the correct form are excluded from the list of options:
	            if (wertiview.rusparticiples.types[j].toLowerCase() != $hit.attr('correctForm').toLowerCase() && wertiview.rusparticiples.types[j] != "") 
	            {
	               options.push(wertiview.lib.matchCapitalization(wertiview.rusparticiples.types[j], capType)); 
	            }
				j++;
			}
