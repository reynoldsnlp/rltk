wertiview.ns(function() {
	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewhit').addClass('colorizeStyleRusVerbPastTense');
	},
	
	colorizeSpan: function(span, topic, index) {
		span.find('span.wertiviewhit').addClass('colorizeStyleRusVerbPastTense');
	},
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.rusverbpasttense.clickHandler)); 
	},
	...
	mc: function(contextDoc) {
		...
		var hitList = [];
		var tokens = [];
		wertiview.rusverbpasttense.types = [];
		...
		wertiview.rusverbpasttense.maxLength = wertiview.rusverbpasttense.MAX_MC;
		wertiview.activity.mc(contextDoc, hitList, 
		wertiview.rusverbpasttense.clozeInputHandler, 
		wertiview.rusverbpasttense.clozeHintHandler, 
		wertiview.rusverbpasttense.mcGetOptions, 
		wertiview.rusverbpasttense.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.rusverbpasttense.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.rusverbpasttense.types);
		        
		        // Add the distractor forms to the options list:
		        while (j < wertiview.rusverbpasttense.types.length && options.length < wertiview.rusverbpasttense.MAX_MC - 1) {
		            // The forms that are homonymous to the correct form are excluded from the list of options:
		            if (wertiview.rusverbpasttense.types[j].toLowerCase() != $hit.attr('correctForm').toLowerCase() && wertiview.rusverbpasttense.types[j] != "") 
		            {
		               options.push(wertiview.lib.matchCapitalization(wertiview.rusverbpasttense.types[j], capType)); 
		            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.rusverbpasttense.clozeHintHandler, 
		wertiview.rusverbpasttense.mcGetCorrectAnswer,
		wertiview.rusverbpasttense.clozeAddBaseform);
	},
