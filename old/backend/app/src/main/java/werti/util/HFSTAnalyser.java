 ...
 *
 * Specifically the HFSTAnalyser expects a word form
...
public class HFSTAnalyser {
    ...
    /**
     * The constructor needed to activate the hfst-analyser for a
     * given {@code language}.
     *
     * @param resourceFactory the factory the hfst-analyser is stored in
     * @param language the language for the hfst-analyser
     * @throws NoResourceException when there is a problem reading the file
     * defined in VIEW.properties with hfst.analyser.{@code language}
     */
    public HFSTAnalyser(
            @Nonnull FileBackedResourceFactory resourceFactory,
            final Language language
    ) throws NoResourceException {
        final CachingResource resource = resourceFactory.create("hfst.analyser");
