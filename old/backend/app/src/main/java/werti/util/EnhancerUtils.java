...
public class EnhancerUtils {
    private EnhancerUtils() { } // avoid instantiation

    private static final Logger log = LogManager.getLogger();

    public static final String addedSpanStyle = "display: inline; background-image: none; padding: 0px; margin: 0px; color: inherit; font: inherit; font-size: 100%; position: relative; top: 0px; left: 0px; background-color: inherit;";
    ...
    /**
     * Predicate to check if annotations are on punctuation alone.
     * Only 1-character wide annotations can be considered punctuation here.
     *
     * @return True iff the covered text is punctuation
     */
    public static boolean isPunctuation(final Annotation annotation) {
    ...
    public static String casToEnhanced(JCas cas, String activity) {
        log.debug("Starting conversion of CAS to <e>");
        ...
        log.debug("Finished conversion of CAS to <e>");
        return rtext.toString();
    }

    @SuppressWarnings("unchecked")
    public static HashMap<Integer, String> getInsertedTags(JCas cas, String activity) {
        final FSIndex tagIndex = cas.getAnnotationIndex(EnhancementAnnotation.type);
        final Iterator<EnhancementAnnotation> eit = tagIndex.iterator();
        ...
        while (eit.hasNext()) {
            EnhancementAnnotation e = eit.next();

            if (activity.matches("click")) {
