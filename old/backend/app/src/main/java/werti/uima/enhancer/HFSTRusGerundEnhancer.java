 ...
 *
 */
public class HFSTRusGerundEnhancer extends AbstractHFSTRussianEnhancer {
	private static final String TOPIC = "gerunds";

    // FSTUPDATE: The patterns target FST features to find or exclude specific readings
    private final Pattern posPattern = Pattern.compile("\\+Adv");
    private final Pattern filterPattern = Pattern.compile("PrsAct|PstAct");
    // Note that the whole token will be excluded, even when one reading is valid
    private final Pattern excludeTokenPattern = Pattern.compile("\\+N\\+|A\\+|Det|Pron|Pcle|Interj|CC|CS|Pred");
