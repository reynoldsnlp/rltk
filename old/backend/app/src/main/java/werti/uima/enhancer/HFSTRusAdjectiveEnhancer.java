 ...
 *
 */
public class HFSTRusAdjectiveEnhancer extends AbstractHFSTRussianEnhancer {
	private static final String TOPIC = "adjectives";

    // FSTUPDATE: The patterns target FST features to find or exclude specific readings
    private final Pattern posPattern = Pattern.compile("A\\+");
    private final Pattern filterPattern = Pattern.compile("Fem|Msc|Neu|MFN");
    // Note that the whole token will be excluded, even when one reading is valid
    private final Pattern excludeTokenPattern = Pattern.compile("Pred");
