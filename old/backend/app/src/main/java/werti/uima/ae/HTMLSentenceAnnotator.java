...
public class HTMLSentenceAnnotator extends JCasAnnotator_ImplBase {
	...
	public void process(JCas jcas) throws AnalysisEngineProcessException {
		...
		final AnnotationIndex rtIndex = jcas.getAnnotationIndex(Block.type);
				
		final Iterator<PlainTextSentenceAnnotation> sit = sentIndex.iterator();
		
		while (sit.hasNext()) {
			final PlainTextSentenceAnnotation s = sit.next();
			final Iterator<Block> rtit = rtIndex.subiterator(s, true, false);
			...
			while (rtit.hasNext()) {
				...
				final String currContentType = t.getBlockName();
			...
			// corresponded to a single Block span or all spans were
