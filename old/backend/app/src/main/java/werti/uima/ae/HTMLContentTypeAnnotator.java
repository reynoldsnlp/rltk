...
public class HTMLContentTypeAnnotator extends JCasAnnotator_ImplBase {
	...
	public void process(JCas cas) throws AnalysisEngineProcessException {
		...
		final Iterator<Block> rtIt1 = rtIndex.iterator();
		...
		final Iterator<Block> rtIt = rtIndex.iterator();
		...
		final Iterator<Block> rtIt2 = rtIndex.iterator();
		for (int i=0; i<secondDataset.numInstances(); i++){
			...
			rt.setBlockName(classification);
		}
	 ...
	 * @param rt this element's corresponding Block annot. object
	 */
...
//----------------------------------------------------------------------------
	 ...
	 * @param rtIt1 an iterator over the Block chunks, for tokenization
	 */
	private void docCounts(Element e, boolean isInA, Elements wertiTextSpans, 
			Iterator<Block> rtIt1){
	 ...
	 */
	private double linkDensity(Elements parents, Block rt){
	 ...
	 */
	@SuppressWarnings("unchecked")
	private List<Token> tokenize(Block rt){
	 ...
	 */
	@SuppressWarnings("unchecked")
	private List<PlainTextSentenceAnnotation> sentDetect(Block rt){
