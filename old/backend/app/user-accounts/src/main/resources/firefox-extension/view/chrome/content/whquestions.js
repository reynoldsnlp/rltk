wertiview.ns(function() {
	...
	remove: function(contextDoc) {
		...
		// remove click
		$('body').undelegate('span.wertiviewchunk', 'click');
		...
		$('body').undelegate('span.wertiviewQ span.wertiviewchunk', 'click');
		$('body').undelegate('span.wertiviewQ span.wertiviewtoken', 'click');
		$('body').undelegate('span.wertiviewQ span.wertiviewrestofq', 'click');
		$('body').undelegate('span.wertiviewcheck', 'click');
		$('body').undelegate('span.wertiviewclear', 'click');
		$('body').undelegate('span.wertiviewhint', 'click');
		$('body').undelegate('input.wertiviewinput', 'keyup keydown blur');
	},
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewchunk', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.whquestions.clickChunkHandler));
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.whquestions.clickTokenHandler));
	},

	clickChunkHandler: function(element, event) {
	...
	clickTokenHandler: function(element, event) {
		var contextDoc = event.data.context;
		...
		if ($(element).parents('.wertiviewQ').length == 0) {
			...
			if (chunkElements.length > 0) {
				countsAsCorrect = wertiview.whquestions.clickChunkHandler(chunkElements.first(), event);
			}
		}

		// allow event to propagate up to the wertiviewchunk level
		return countsAsCorrect;
	},
	...
	cloze: function(contextDoc) {
		...
		$('body').delegate('span.wertiviewcheck', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.whquestions.clozeCheckHandler, false, wertiview.whquestions.clozeGetInput, wertiview.whquestions.clozeGetCorrectAnswer));
		$('body').delegate('span.wertiviewclear', 'click', {context: contextDoc}, wertiview.whquestions.clozeClearHandler);
		$('body').delegate('span.wertiviewhint', 'click', {context: contextDoc}, wertiview.activity.getHandler(wertiview.whquestions.clozeHintHandler, true, wertiview.whquestions.clozeGetInput, wertiview.whquestions.clozeGetCorrectAnswer));
	...
	clozeCheckHandler: function(element, event) {
		var contextDoc = event.data.context;
