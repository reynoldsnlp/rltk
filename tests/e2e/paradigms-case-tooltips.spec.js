...
test.describe('Paradigm case tooltips', () => {
  ...
  test('short adjective label exposes tooltip content', async ({ page, browserContext, extensionId }, testInfo) => {
    const baseURL = testInfo.project.use.baseURL;
    const fixtureUrl = `${baseURL}/tests/fixtures/comprehensive-pos.html`;
    ...
    const target = page.locator('#adj-big .ʁ-reading-tutor').first();
    await expect(target).toBeVisible({ timeout: 10000 });
    await target.click();
    ...
    const shortLabel = sidePanelPage.locator('.paradigm-table .case-tooltip', { hasText: 'Short' }).first();
    await expect(shortLabel).toBeVisible({ timeout: 20000 });
    await shortLabel.hover();

    const tooltip = shortLabel.locator('.case-tooltip-content');
    await expect(tooltip).toBeVisible({ timeout: 10000 });
    await expect(tooltip).toContainText('Short-form');
  });
});
