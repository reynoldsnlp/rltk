# E2E Test Strategy & Flakiness Notes
...
- **`fullyParallel: true`**; parallelism is governed by `playwright.config.js`:
  ...
  - **CI:** `workers: 1`, `retries: 2` (memory-safe on the runner).
  - Global `timeout: 60000`.
- **`scripts/run-playwright.sh` no longer hardcodes `--workers`/`--retries`** — the
  config governs. `"$@"` still lets you override (e.g. `npm run test -- --workers=2`).
...
2. **Never `page.waitForTimeout(N)`.** Replace every fixed sleep with a wait on a
   ...
   on a deterministic precondition instead of sleeping — see `refresh:45`.)
3. **Wait for actionability before acting; wait for settled before asserting a
   resting state.** (The `:178` lesson — don't assert the final button label while
...
4. **Make transient-state tests deterministic — don't race analysis speed.** Use the
   ...
   not real timing. (The `:104` lesson.)
...
of the `:104` timeout. `timeout: 60000` in config; tests still raise further with
`test.setTimeout()`.
...
- **Spurious dirty after re-analysis:** the dirty observer re-flagged the page after a
  ...
  — "dirty" now means real unanalyzed Cyrillic remains. (Fixed `:178` at the root.)
...
### ❌ Fixing `:104` with bigger timeouts / a "batch-2" pause anchor alone
...
### ❌ Fixing `:178` with a test-side settle wait alone
...
- [ ] Only use `mode: 'serial'` if the tests genuinely depend on order/shared state (almost never — `beforeEach` clears storage).
