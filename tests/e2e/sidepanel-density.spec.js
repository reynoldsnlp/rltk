...
test.describe('Side panel density for MC/Cloze', () => {
  ...
  test('saves density in side panel and applies to nouns MC/Cloze', async () => {
    ...
    await expect(mcSpan).toBeVisible({ timeout: 5000 });
    ...
    await page.waitForFunction(() => document.querySelectorAll('.ʁ-noun-mc').length === 0, { timeout: 5000 });
    await page.waitForFunction(() => document.querySelectorAll('.ʁ-noun-mc').length > 0, { timeout: 5000 });
    ...
    await expect(clozeSpan).toBeVisible({ timeout: 5000 });
  });
});
